...
class MKOROptimizer(optim.Optimizer):
    ...
    def compute_factorization_costs(self):
        worker_costs = [0] * dist.get_world_size()
    ...
    def inverse(self, prev_inv, rank_1):
        ...
        # a = torch.abs(1 / (1 + tmp2 @ rank_1) * tmp1 @ tmp2)
        # b = torch.abs(prev_inv - 1 / (1 + tmp2 @ rank_1) * tmp1 @ tmp2)
        # print(b.max(), a.max(), rank_1.abs().max())
        return prev_inv - 1 / (1 + tmp2 @ rank_1) * tmp1 @ tmp2

    def _save_input(self, module, input):
        if (torch.is_grad_enabled() and (self.steps % self.inv_freq == 0 or self.steps < self.warmup_steps)) and self.kfac:
    ...
    def _save_grad_output(self, module, grad_input, grad_output):
        ...
        if (self.steps % self.inv_freq == 0 or self.steps < self.warmup_steps) and self.kfac:
    ...
    def _update_inv(self, m):
        ...
        self.d_a[self.index(m)], self.Q_a[self.index(m)] = torch.linalg.eigh(
            self.m_aa[self.index(m)] + eps / 10 * torch.eye(self.m_aa[self.index(m)].shape[0],
                                                            device=self.m_aa[self.index(m)].device))
        self.d_g[self.index(m)], self.Q_g[self.index(m)] = torch.linalg.eigh(
            self.m_gg[self.index(m)] + eps / 10 * torch.eye(self.m_gg[self.index(m)].shape[0],
                                                            device=self.m_gg[self.index(m)].device))
    ...
    def _get_natural_grad(self, m, p_grad_mat, identity=False):
        ...
        if identity:
            ...
            if self.rank == 1:
                ...
                v = self.rank * (self.GG_Us[self.index(m)] @ (
                            self.GG_Vt[self.index(m)] @ p_grad_mat @ self.AA_Us[self.index(m)]) @ self.AA_Vt[
                                     self.index(m)]) + (
    ...
    def reduce_data(self, data_dict, distributed=False, distribution_dict=None):
        ...
        if distributed:
            data_list = [[] for _ in range(dist.get_world_size())]
            data_shapes = [[] for _ in range(dist.get_world_size())]
            for module in self.modules:
                if self.apply_sgd[self.index(module)]:
                    continue
            ...
            reduced_data = [torch.cat(data_list[i], dim=1) for i in range(dist.get_world_size())]
            del data_list
            handles = []
            for i in range(dist.get_world_size()):
                handles.extend(self.reduce(reduced_data[i], dst=i, async_op=True, average=True))
    ...
    def invert_factor(self, module, original_factor_dict, factor_dict, a, v):
        if self.method == 'exact':
            ...
            # print("Before", factor_dict[self.index(module)].abs().max(), v.abs().max())
            ...
            factor_dict[self.index(module)] = self.inverse(0.95 * factor_dict[self.index(module)] + 0.05 * torch.eye(shape[0], device=self.device, dtype=self.data_type),
                                                           v)
            # print("After", factor_dict[self.index(module)].abs().max(), factor_dict[self.index(module)].abs().mean())
    ...
    def set_reset_factor_flags(self, module, factor_dict):
        if self.manual_reset_factors:
            ...
            # max = torch.max(torch.abs(factor_dict[self.index(module)].flatten()))
            # mean = torch.mean(torch.abs(factor_dict[self.index(module)].flatten()))
            # print(max, mean, max / mean)
            if torch.max(torch.abs(factor_dict[self.index(module)].flatten())) > 2:
                print(torch.max(torch.abs(factor_dict[self.index(module)].flatten())))
    ...
    def reduce_and_update_factors(self):
        if self.kfac:
            ...
            for module in self.modules:
                classname = module.__class__.__name__
                p_grad_mat = self.timer("precondition", self._get_matrix_form_grad, m=module, classname=classname)
                self.grads[self.index(module)] = p_grad_mat.mean(dim=1).reshape(1, -1)
                self.inputs[self.index(module)] = p_grad_mat.mean(dim=0).reshape(1, -1)
                # print(self.grads[self.index(module)].shape, self.inputs[self.index(module)].shape)
    ...
    def compute_min_eigenvals(self):
        ...
        for module in self.modules:
            if self.apply_sgd[self.index(module)]:
                continue
            num_mkor_layers += 1
            norm = torch.norm(self.AA_inv[self.index(module)] - torch.eye(
                self.AA_inv[self.index(module)].size(0)).cuda()) / torch.numel(
                self.AA_inv[self.index(module)])
            print(norm)
            if norm < 1e-6:
                inv_factor = self.AA_inv[self.index(module)].to(torch.float32)
            else:
                inv_factor = (self.AA_inv[self.index(module)] - torch.eye(
                    self.AA_inv[self.index(module)].size(0)).cuda()).to(torch.float64)
            density_ratios_a += torch.sum(
                torch.abs(self.AA_inv[self.index(module)] - torch.eye(
                    self.AA_inv[self.index(module)].size(0)).cuda()) > 5e-2) / torch.numel(
                self.AA_inv[self.index(module)])
            density_ratios_g += torch.sum(
                torch.abs(self.GG_inv[self.index(module)] - torch.eye(
                    self.GG_inv[self.index(module)].size(0)).cuda()) > 5e-2) / torch.numel(
    ...
    def compute_sparse_preconditioning_costs(self):
        ...
        self.broadcast(left_thresholds, src=0, async_op=False)
        self.broadcast(right_thresholds, src=0, async_op=False)
