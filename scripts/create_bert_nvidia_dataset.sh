...
VOCAB_FILE=$DATA_PATH/google_pretrained_weights/uncased_L-24_H-1024_A-16/vocab.txt
...
if [ "$DOWNLOAD" == true ]; then
    python data_loader/utils/download.py --dir $DATA_PATH --datasets wikicorpus
    python data_loader/utils/download.py --dir $DATA_PATH --datasets squad
    python data_loader/utils/download.py --dir $DATA_PATH --datasets weights
    if [ "$INCLUDE_BOOKS" == true ]; then
        python data_loader/utils/download.py --dir $DATA_PATH --datasets bookscorpus
    fi
fi
...
if [ "$FORMAT" == true ]; then
    ...
    mkdir -p $DATA_PATH/wikicorpus/data
    wikiextractor $DATA_PATH/wikicorpus/wikicorpus_en.xml --bytes 25M \
        --processes $N_PROCESSES --output $DATA_PATH/wikicorpus/data
    ...
    python data_loader/utils/format.py --dataset wikicorpus --processes $N_PROCESSES \
        --input_dir $DATA_PATH/wikicorpus/data \
        --output_dir $FORMAT_PATH/wikicorpus --shards 256
    if [ "$INCLUDE_BOOKS" == true ]; then
        python data_loader/utils/format.py --dataset bookscorpus --processes $N_PROCESSES \
            --input_dir $DATA_PATH/bookscorpus/data \
...
if [ "$ENCODE" == true ]; then
    if [ "$ENCODE_TYPE" == "roberta" ]; then
        ...
        python data_loader/utils/encode_pretraining_data.py \
        ...
        python data_loader/utils/encode_data.py \
            --input_dir $FORMAT_PATH --output_dir $ENCODED_PATH \
