...
ACT2FN = {"gelu": gelu, "bias_gelu": bias_gelu, "bias_tanh": bias_tanh, "relu": torch.nn.functional.relu,
          "swish": swish}
...
class BertForNextSentencePrediction(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, next_sentence_label=None, **kwargs):
...
class BertForMultipleChoice(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None, **kwargs):
...
class BertPretrainingCriterion(torch.nn.Module):
    ...
    def forward(self, logits, labels,
        ...
        masked_lm_loss = self.loss_fn(logits.view(-1, self.vocab_size),
                                      labels.view(-1))
...
class BertForQuestionAnsweringCriterion(torch.nn.Module):
    def __init__(self):
        super(BertForQuestionAnsweringCriterion, self).__init__()
...
class BertForSequenceClassificationCriterion(torch.nn.Module):
    def __init__(self):
        super(BertForSequenceClassificationCriterion, self).__init__()
        self.loss_fn = torch.nn.CrossEntropyLoss()

    def forward(self, logits, labels):
        return self.loss_fn(logits.view(-1, 2), labels.view(-1))